    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        selectCompleteBtn.setOnClickListener(item -> {
            if (selected_menu.size() == 1) {
                preferences = getContext().getSharedPreferences("selected_diet_kimchi", Context.MODE_PRIVATE);
                SharedPreferences.Editor editor = preferences.edit();
                editor.putString("selected_diet_kimchi", selected_menu.get(0).getItem_index().toString());
                editor.commit();
                ...
                for (int i = 0; i < selected_menu.size() - 1; i++) {
                    ...
                    if (selected_menu.size() >= 1) {
                        editor.putString("selected_diet_kimchi", selected_menu.get(selected_menu.size() - 1).getItem_index().toString());
                        editor.commit();
                    }
                }
            }
